  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/user/likespu/info/${this.dataForm.id}`),
            method: 'get',
            params: this.$http.adornParams()
          }).then(({data}) => {
            if (data && data.code === 0) {
              this.dataForm.userId = data.likeSpu.userId
              this.dataForm.spuId = data.likeSpu.spuId
              this.dataForm.spuName = data.likeSpu.spuName
              this.dataForm.spuImg = data.likeSpu.spuImg
              this.dataForm.createTime = data.likeSpu.createTime
            }
          })
        }
      })
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/user/likespu/${!this.dataForm.id ? 'save' : 'update'}`),
            method: 'post',
