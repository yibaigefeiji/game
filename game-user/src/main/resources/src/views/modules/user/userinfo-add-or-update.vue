  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/user/userinfo/info/${this.dataForm.id}`),
            method: 'get',
            params: this.$http.adornParams()
          }).then(({data}) => {
            if (data && data.code === 0) {
              this.dataForm.userId = data.userInfo.userId
              this.dataForm.orderCount = data.userInfo.orderCount
              this.dataForm.couponCount = data.userInfo.couponCount
              this.dataForm.commentCount = data.userInfo.commentCount
              this.dataForm.attendCount = data.userInfo.attendCount
              this.dataForm.fansCount = data.userInfo.fansCount
              this.dataForm.collectProductCount = data.userInfo.collectProductCount
            }
          })
        }
      })
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/user/userinfo/${!this.dataForm.id ? 'save' : 'update'}`),
            method: 'post',
